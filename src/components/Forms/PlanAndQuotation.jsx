...
const PlanAndQuotation = () => {
  ...
  const handleNext = () => {
    if (activeStep === 1) {
      if (!formData.plan) { setErrorPlan(true); return; }
      if (metodoUbicacion === 'manual' && !formData.calleNumero.trim()) { setErrorDireccion(true); return; }
    }
        ...
        return (
          <Stack spacing={3} sx={{ mt: 2 }}>
            ...
            <Box sx={{ p: 2, backgroundColor: '#f8fafc', borderRadius: 2, border: '1px solid #e2e8f0' }}>
              ...
              <Stack spacing={3}>
                <TextField label="Referencias" multiline rows={2} fullWidth size="small" value={formData.referencias} onChange={(e) => setFormData({...formData, referencias: e.target.value})} />
                <TextField label="Detalles de Fachada" multiline rows={2} fullWidth size="small" value={formData.detallesCasa} onChange={(e) => setFormData({...formData, detallesCasa: e.target.value})} />
              </Stack>
            </Box>
          </Stack>
        );
        ...
        return (
          <Box sx={{ mt: 2 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} sx={{ mb: 3 }}>
              <Button variant="outlined" color="primary" component="label" startIcon={<AddPhotoAlternate />} fullWidth>Foto Frente INE<input type="file" hidden accept="image/*" capture="environment" /></Button>
              <Button variant="outlined" color="primary" component="label" startIcon={<AddPhotoAlternate />} fullWidth>Foto Reverso INE<input type="file" hidden accept="image/*" capture="environment" /></Button>
            </Stack>
            <Paper variant="outlined" sx={{ p: 2, bgcolor: '#f8fafc', mb: 3, borderRadius: 2 }}>
              ...
              {formData.plan && (<Box sx={{ mb: 2, p: 1.5, backgroundColor: '#e3f2fd', borderRadius: 1, borderLeft: '3px solid #2196F3' }}><Typography variant="body2" sx={{ fontWeight: 600, color: '#1565c0' }}>Plan: <strong>{formData.plan.nombre}</strong></Typography></Box>)}
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}><Typography variant="body2">Instalación:</Typography><Typography variant="body2">${totales.instalacion.toFixed(2)}</Typography></Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}><Typography variant="body2">Primer mes:</Typography><Typography variant="body2">${totales.primerMes.toFixed(2)}</Typography></Box>
              <Divider sx={{ mb: 1 }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}><Typography variant="subtitle2" sx={{ fontWeight: 700 }}>Total:</Typography><Typography variant="subtitle2" sx={{ fontWeight: 700, color: '#16a34a' }}>${totales.total.toFixed(2)}</Typography></Box>
            </Paper>
            <Alert severity="info" icon={<InfoOutlined />} sx={{ mb: 3 }}>Tiempo estimado: **3 a 5 días hábiles**.</Alert>
            <Typography variant="subtitle2" sx={{ fontWeight: 700, mb: 1 }}>Firma del Cliente</Typography>
            <Box sx={{ backgroundColor: '#fff', border: '2px dashed #cbd5e1', borderRadius: 2, height: 200, touchAction: 'none' }}><canvas ref={canvasRef} width={800} height={200} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} onMouseLeave={stopDrawing} onTouchStart={startDrawing} onTouchMove={draw} onTouchEnd={stopDrawing} style={{ width: '100%', height: '100%', cursor: 'crosshair' }} /></Box>
