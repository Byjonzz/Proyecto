...
// Importaciones de Leaflet (Se agrega GeoJSON para dibujar las colonias)
...
const AsignacionRutas = () => {
  ...
  // === BUSCADOR INTELIGENTE DE COLONIAS Y LÍMITES ===
  const handleBuscarUbicacion = async () => {
    if (!busquedaColonia.trim()) return;
    
    // Si el usuario no escribe "Tehuacán", el sistema lo agrega automáticamente para evitar errores
    const terminoBusqueda = busquedaColonia.toLowerCase().includes('tehuacán') 
      ? busquedaColonia 
      : `${busquedaColonia}, Tehuacán`;

    const query = `${terminoBusqueda}, Puebla, México`;
    
    // Agregamos polygon_geojson=1 para pedirle al servidor las fronteras de la colonia
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        // 1. Mueve el mapa al centro de la colonia
        if (resultado.lat && resultado.lon) {
          setCentroMapa([parseFloat(resultado.lat), parseFloat(resultado.lon)]);
        }

        // 2. Si el servidor tiene dibujadas las fronteras, las guarda para pintarlas en morado
        if (resultado.geojson) {
          setLimiteColonia(resultado.geojson);
        } else {
          setLimiteColonia(null); // Limpia si no hay fronteras disponibles
        }
      } else {
        alert("No se encontró la colonia. Intenta ser más específico o revisar la ortografía.");
  ...
  const handleGuardarRuta = () => {
    ...
    setCanvaceador('');
    setBusquedaColonia('');
    setLimiteColonia(null); // Limpiamos el polígono morado
    limpiarMapa();
  };
  ...
  const MapCenterUpdater = ({ center }) => {
    ...
    useEffect(() => {
      if (center && center.length === 2) {
        // Hacemos zoom a 16 para acercarnos más a la colonia
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3, width: '100%' }}>
      ...
      <Box>
        ...
        <Typography variant="body2" color="text.secondary">
          Busca una colonia, revisa sus fronteras visuales y asigna la ruta peatonal al equipo.
        </Typography>
      </Box>
            ...
            <CardContent sx={{ p: 3 }}>
              ...
              <Stack spacing={3}>
                ...
                <Alert severity="info" sx={{ '& .MuiAlert-message': { fontSize: '0.75rem' } }}>
                  El motor calculará el camino peatonal más corto utilizando cruces, calles y callejones oficiales.
                </Alert>
              </Stack>
            </CardContent>
          </Card>
            ...
            <Card variant="outlined" sx={{ borderRadius: 3, border: '2px solid #3b82f6', overflow: 'hidden', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
              ...
              <Box sx={{ width: '100%', height: 480, position: 'relative' }}>
                ...
                >
                  ...
                  {limiteColonia && (
                    <GeoJSON 
                      key={JSON.stringify(limiteColonia)} // Obliga al mapa a redibujar cuando cambias de colonia
                      data={limiteColonia} 
                      style={{ color: '#8b5cf6', weight: 3, fillColor: '#8b5cf6', fillOpacity: 0.15 }}
                    >
                      <Tooltip sticky>Fronteras de la Colonia</Tooltip>
                    </GeoJSON>
                  )}
                ...
                {(!Array.isArray(puntosRuta) || puntosRuta.length === 0) && (
                  <Box sx={{ position: 'absolute', top: 20, left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(255,255,255,0.95)', px: 3, py: 1.5, borderRadius: 3, pointerEvents: 'none', boxShadow: '0 4px 10px rgba(0,0,0,0.15)', zIndex: 1000 }}>
                    <Typography variant="body2" sx={{ color: '#334155', fontWeight: 700, textAlign: 'center' }}>
                      Haz clics en las calles para crear los destinos del canvaceador
                    </Typography>
                  </Box>
                )}
              </Box>
